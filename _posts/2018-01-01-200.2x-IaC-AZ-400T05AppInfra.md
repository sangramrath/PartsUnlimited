...
We will perform these steps in the Azure CLI, and we would recommend to do so for ease of use. Azure Cloud Shell, has the latest **Az** powershell module installed ready for use. however you could manually create a blob container via the Azure Portal, upload the file and generate a URL, or run the powershell commands locally, however ensure you have the latest powershell modules installed locally if running from a local environment.
...
3. Click **Upload/Download files** button on the shell and upload the `storage.json` file created earlier.

4. The below is a PowerShell script, that will create a blob storage container, upload our template file and generate s URL with a SAS token that we can call in our main template. Have a read through the below to understand what it is doing. When you are ready copy and paste the below into the **Azure Cloud Shell** and enter values as prompted. i.e.
    

    **Note**: Ensure you note the value of the output at the end of the script, as they will be required later in the lab.


    ```powershell
    $projectNamePrefix = Read-Host -Prompt "Enter a name:"   # This name is used to generate names for Azure resources, such as storage account name.
